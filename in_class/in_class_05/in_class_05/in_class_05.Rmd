...
# A: District of Columbia; 13,959 per county (ward?)
```
...
```{r}
# Q: Which state had the highest total number of deaths? 
most_deaths_state <- covid_county %>%
  group_by(state)%>%
...
```{r}
# Q: Which state had the lowest total number of deaths?
least_deaths_state <- covid_county %>%
  group_by(state)%>%
  summarise(total_deaths = sum(deaths))%>%
