...
``` {r}
covid_state_case_rate_august <- state_covid_population %>%
  select(state, total_cases, population) %>%
  mutate(cases_per_1000_august = (total_cases/population)*1000) %>%
  arrange(desc(cases_per_1000_august))

# Print below
covid_state_case_rate_august

#A: Louisiana, 31.667 cases per 1,000 people
```
...
```{r}
covid_state_death_rate_august <- state_covid_population %>%
  select(state, total_deaths, population) %>%
  mutate(deaths_per_1000_august = (total_deaths/population)*1000) %>%
  arrange(deaths_per_1000_august)

# Print below
covid_state_death_rate_august

#A: Hawaii, with .0436 deaths per 1000
```
